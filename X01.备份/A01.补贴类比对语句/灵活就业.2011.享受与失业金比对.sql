 ...
 select
  ...
  select AAC001, AJC150 ksny, AJC150 jsny
        from JC03$ a
       where CJC016 is null 
)       
...
select  XSNY 未参保年月,AAC002 身份证,AAC003 姓名,AAE017 办理机构,JE,XSNF,XSND,FFBZ,FFNF,FFND,CCC018,AAB003,AAC001 人员内码
  from  bb
 --where  xsny > txny --退休
 --where not exists(select * from lev_ac07 cc where cc.AAC001=bb.AAC001 and  AAE030=xsny) --参保
 where exists(select * from cc where cc.aac001=bb.aac001 and cc.ksny=bb.xsny)
